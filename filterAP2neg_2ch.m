function [] = filterAP2neg_2ch(data, varargin)
%  AP2_NEG Labels = 2
%  AP2_POS Labels = 5
%  catidx = 1
% Ti2E untitledfilter 1.08 maxfilter 1.18 SNR 1.05 1.6
ip = inputParser;
...
ip.addParameter('A_SNR',[1.1 1.3],@isnumeric); %Artificial definition SNR
ip.addParameter('lifetimeThreshold',[5 inf],@isnumeric);%minlifetime 5s maxlifetime inf
...
lifetime_t = ip.Results.lifetimeThreshold;
...
parfor nd = 1:length(data)
    ...
    for i = 1:length(tracks)
        smooth_A = (smooth(tracks(i).A(1,:)))'; 
        Signalmch = smooth_A+tracks(i).c(1,:);
        Signalmchlow = smooth_A+tracks(i).c(1,:)-tracks(i).A_pstd(1,:);
        ...
        if  maskfilter && lifetimefilter && snrfilter && gapfilter && a_defined_filter && max_firstfilter && intensityfilter
            idx = [idx i];
        end
    end
    tracks = tracks(idx);
   [tmp_neg_index,tmp_neg_longindex,tmp_pos_index]=deal([]); 
    ...
    if  length(tracks_neg)>=1
        ...
        [PLC_AP2neg_summary(nd).maxA,PLC_AP2neg_summary(nd).Lft,PLC_AP2neg_summary(nd).X,PLC_AP2neg_summary(nd).Y,PLC_AP2neg_summary(nd).Totaldisplacement,PLC_AP2neg_summary(nd).firstNmaxA] = ...
            deal(tmp_tmp.maxA,tmp_tmp.Lft,tmp_tmp.X,tmp_tmp.Y,tmp_tmp.Totaldisplacement,tmp_tmp.firstNframes_maxA);
    ...
    if length(tracks_neg_long)>=1
        ...
        [PLC_AP2negLong_summary(nd).maxA,PLC_AP2negLong_summary(nd).Lft,PLC_AP2negLong_summary(nd).X,PLC_AP2negLong_summary(nd).Y,PLC_AP2negLong_summary(nd).Totaldisplacement,PLC_AP2negLong_summary(nd).firstNmaxA] = ...
            deal(tmp_tmp.maxA,tmp_tmp.Lft,tmp_tmp.X,tmp_tmp.Y,tmp_tmp.Totaldisplacement,tmp_tmp.firstNframes_maxA);
    ...
    if length(tracks_pos)>=1
        ...
        [PLC_AP2pos_summary(nd).maxA,PLC_AP2pos_summary(nd).Lft,PLC_AP2pos_summary(nd).X,PLC_AP2pos_summary(nd).Y,PLC_AP2pos_summary(nd).Totaldisplacement,PLC_AP2pos_summary(nd).firstNmaxA] = ...
            deal(tmp_tmp.maxA,tmp_tmp.Lft,tmp_tmp.X,tmp_tmp.Y,tmp_tmp.Totaldisplacement,tmp_tmp.firstNframes_maxA);
...
fprintf(info,['ASNR is [' num2str(asnr(1)) ' ' num2str(asnr(2)) ']\n']);
fclose(info);
